...
func TestClient_GetMarketTrades(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()

	// Exact URL match
	httpmock.RegisterResponder("GET", "http://localhost/v1/market/LTC_BTC/trades",
		httpmock.NewStringResponder(200, marketTradesTestData))

	wantTime, _ := time.Parse(time.RFC3339Nano, "2019-05-21T02:41:30.781308Z")
	...
	got, err := testClient.GetMarketTrades(context.Background(), LTC_BTC)
	if assert.NoError(t, err) {
		assert.Equal(t, want, got)
	}
